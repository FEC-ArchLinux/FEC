...
function ProductDetails({ productInfo, styles, activeStyle, productId }) {
  ...
  function getProductReviewData() {
    axios
      .get(
        `https://app-hrsei-api.herokuapp.com/api/fec2/hr-rfe/reviews/meta?product_id=${productId}`,
        {
          headers: {
  ...
  function priceGenerator() {
    if (styles[activeStyle].sale_price) {
      return (
        <>
          <p style={{ margin: 0 }}>
            $<s>{styles[activeStyle].original_price}</s>
          </p>
          <p style={{ margin: 0, color: "red" }}>
            ${styles[activeStyle].sale_price}
          </p>
        </>
      );
    }
  ...
  return (
    <div>
      <StarRatings
        ...
        numberOfStars={5}
        starSpacing="2px"
        starDimension="calc(.3vw + .3vh + 7px)"
        rating={starRating ? Math.round(starRating * 100) / 100 : 0}
      />
      <br />
      <ReviewSpan
        onClick={() =>
      ...
      >
        See all {starRating && totalReviews} reviews
      </ReviewSpan>
